...
public class ScalableVideoView extends TextureView implements TextureView.SurfaceTextureListener,
    ...
    public int FORCED_WIDTH = 0;
    public int FORCED_HEIGHT = 0;
    ...
    public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
        if (FORCED_WIDTH > 0 && FORCED_HEIGHT > 0) {
            scaleVideoSize(FORCED_WIDTH, FORCED_HEIGHT);
        }
        else {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        float aspectRatio = (float) getVideoHeight() / (float) getVideoWidth();
        float height = (float) (w) * aspectRatio;
        scaleVideoSize(w, (int)height);
    }
    ...
    public void prepare(MediaPlayer.OnPreparedListener listener) throws IOException, IllegalStateException {
    ...
    public MediaPlayer.TrackInfo[] getTrackInfos() {
